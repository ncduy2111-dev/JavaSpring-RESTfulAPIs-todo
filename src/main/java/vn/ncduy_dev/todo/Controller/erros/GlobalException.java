package vn.ncduy_dev.todo.Controller.erros;
...
public class GlobalException {
    ...
    public ResponseEntity<ApiResponse<?>> handleNotFound(NoSuchElementException ex) {
        var result = new ApiResponse<>(HttpStatus.NOT_FOUND, "handleNotFound", null, ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(result);
    }

    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<ApiResponse<?>> handleBadRequest(IllegalArgumentException ex) {
        var result = new ApiResponse<>(HttpStatus.BAD_REQUEST, "handleBadRequest", null, ex.getMessage());
